...
import { FileText, Instagram, Mail, Clock, Send } from 'lucide-react';
...
interface ContentCardProps {
  ...
  dueDate: string;
  pillar: string;
  onPublishToSocial?: () => void;
}

const ContentCard: React.FC<ContentCardProps> = ({ title, type, status, dueDate, pillar, onPublishToSocial }) => {
    ...
    >
      ...
      {onPublishToSocial && (
        <button
          ...
          className="mt-3 w-full bg-sage/10 text-sage px-3 py-2 rounded-lg text-xs font-medium hover:bg-sage/20 transition-colors duration-200 flex items-center justify-center space-x-1"
        >
